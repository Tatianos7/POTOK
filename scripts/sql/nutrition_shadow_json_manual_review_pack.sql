-- Nutrition shadow JSON manual review pack
-- Read-only
-- Narrow scope: only the three currently known no-ingredients recipes
-- Purpose:
-- - surface rows that cannot be safely backfilled automatically
-- - show candidate foods when they exist
...
from resolution
where resolution_status <> 'auto_resolvable'
