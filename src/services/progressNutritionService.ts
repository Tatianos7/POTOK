...
export function aggregateNutritionProgress(
  ...
  const deficitVisible = calorieTarget !== null && hasData;
  const deficitValue = deficitVisible ? calorieTarget - totals.calories : null;
  ...
  return {
    ...
    deviations:
      calorieTarget !== null
        ? {
            calories: Math.round(totals.calories - calorieTarget),
          }
        : null,
...
class ProgressNutritionService {
  ...
  async getNutritionProgress(userId: string, anchorDate: string, period: NutritionProgressPeriod): Promise<NutritionStats> {
    ...
    const [rows, goal] = await Promise.all([
      this.deps.diaryRepo.listByUserAndDateRange(userId, start, end),
    ...
    return aggregateNutritionProgress(rows, foodNamesById, anchorDate, period, calorieTarget);
  }
}
