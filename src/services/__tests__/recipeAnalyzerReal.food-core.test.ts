...
test('recipe analyzer resolves Food Core rollout soup example and preserves display units', async () => {
  ...
  try {
    const parsed = parseRecipeText('250 грамм чечевицы, вода 500 мл., лук 1 шт., говядина 300 грамм');
    ...
    assert.deepEqual(
      ...
      [
        { name: 'чечевица', amountText: '250 г', amountGrams: 250, display: '250 г' },
        { name: 'вода', amountText: '500 мл', amountGrams: 500, display: '500 мл' },
        { name: 'лук', amountText: '1 шт', amountGrams: 110, display: '1 шт' },
        { name: 'говядина', amountText: '300 г', amountGrams: 300, display: '300 г' },
      ]
    );

    const analyzed = await analyzeRecipeTextReal(
      '250 грамм чечевицы, вода 500 мл., лук 1 шт., говядина 300 грамм'
    );
    ...
    const onion = analyzed.find((item) => item.name === 'лук');
    assert.equal(onion?.amountText, '1 шт');
    assert.equal(onion?.amountGrams, 110);
