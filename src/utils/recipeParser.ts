...
function parseLine(rawLine: string): ParsedRecipeIngredient | null {
  ...
  cleanedName = cleanedName.replace(/\b(г|гр|кг|л|мл|шт|грамм\w*|литр\w*|миллилитр\w*|штук\w*|дольк\w*|зубчик\w*|ч\.?\s*л\.?|ст\.?\s*л\.?)\b/gi, ''); // удаляем единицы
  // Удаляем возможные ведущие/висячие токены единиц
