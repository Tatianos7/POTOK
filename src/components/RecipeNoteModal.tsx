...
const RecipeNoteModal = ({ isOpen, onClose, onSave, initialNote, onDelete }: RecipeNoteModalProps) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.error('[RecipeNoteModal] Failed to save note:', error);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      console.error('[RecipeNoteModal] Failed to delete note:', error);
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl w-full max-w-md shadow-xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-6 pb-4">
          <h2 className="text-lg font-bold text-gray-900 dark:text-white text-center uppercase">
            {initialNote ? 'РЕДАКТИРОВАТЬ ЗАМЕТКУ' : 'ДОБАВИТЬ ЗАМЕТКУ'}
          </h2>
        </div>
        ...
        <div className="px-6 pb-6">
          <textarea
            ...
            disabled={isSaving}
            placeholder="Здесь вы можете описать способ приготовления блюда"
...
export default RecipeNoteModal;
