...
const ProductCard = ({ food, onClick, onAddClick, isAdded = false }: ProductCardProps) => {
  return (
    <div
      className="w-full max-w-full flex items-stretch gap-2 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 overflow-hidden"
      style={{ padding: '6px' }}
    >
      <button
        onClick={onClick}
        className="flex-1 min-w-0 flex items-start gap-2 min-[376px]:gap-3 rounded-md hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors text-left overflow-hidden"
      >
        {/* Image */}
      ...
      {onAddClick && (
        <button
          ...
          className={`w-10 min-[376px]:w-11 flex-shrink-0 rounded-lg flex items-center justify-center transition-colors ${
            isAdded
